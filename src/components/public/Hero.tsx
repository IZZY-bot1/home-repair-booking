...
export default function Hero({ settings, content, onBookClick }: HeroProps) {
  const businessName = settings?.business_name || 'FixRight Home Repair'
  const badge = content.badge_text || 'Trusted Home Repair Specialists'
  const title = content.title || `Your Home, Expertly Repaired & Maintained`
  const subtitle = content.subtitle || `${businessName} brings professional repair technicians to your door. Schedule a service online in minutes and get your home back in perfect shape.`
  const ctaPrimary = content.cta_primary || 'Book Your Repair'
  const ctaSecondary = content.cta_secondary || 'View Services'
  const stats = [
    { icon: ShieldCheck, value: content.stat1_value || '10+ Years', label: content.stat1_label || 'Experience' },
    { icon: Star, value: content.stat2_value || '4.9 / 5', label: content.stat2_label || 'Client Rating' },
    { icon: Clock, value: content.stat3_value || 'Same-Day', label: content.stat3_label || 'Availability' },
  ]
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
        <div className="max-w-2xl xl:max-w-3xl">
          <div className="inline-flex items-center gap-2 bg-white/12 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5 mb-8 animate-fade-in">
            ...
            <span className="text-white/90 text-sm font-500 tracking-wide">{badge}</span>
          </div>
